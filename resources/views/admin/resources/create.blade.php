...
<div style="max-width: 900px; margin: 0 auto; background: white; padding: 30px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h2 style="color: #2c3e50; margin:0;">Add New Resource</h2>
    ...
    <form action="{{ route('admin.resources.store') }}" method="POST">
        ...
        <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 20px; margin-bottom: 20px;">
            <div>
                ...
                <input type="text" name="name" required placeholder="e.g. Server Alpha-1" style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 4px;">
            </div>
            <div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            <div>
                ...
                <input type="text" name="location" required placeholder="e.g. Rack A, Slot 3" style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 4px;">
            </div>
            <div>
                ...
                <select name="responsable_id" style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 4px; background: white;">
                    ...
                    @foreach($managers as $manager)
                        <option value="{{ $manager->id }}">{{ $manager->name }} ({{ $manager->department ?? 'IT' }})</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div style="margin-bottom: 30px;">
            ...
            <textarea name="description" rows="3" style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 4px; resize: vertical;"></textarea>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            <div>
                ...
                <input type="text" name="cpu" placeholder="e.g. Intel Xeon 8-Core" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
            </div>
            <div>
                <label style="display: block; margin-bottom: 8px; font-weight: 600; color: #555;">RAM Memory</label>
                <input type="text" name="ram" placeholder="e.g. 64GB DDR4" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
            </div>
            <div>
                <label style="display: block; margin-bottom: 8px; font-weight: 600; color: #555;">Storage Capacity</label>
                <input type="text" name="storage" placeholder="e.g. 2TB NVMe SSD" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
            </div>
            <div>
                <label style="display: block; margin-bottom: 8px; font-weight: 600; color: #555;">Operating System</label>
                <input type="text" name="os" placeholder="e.g. Ubuntu 22.04 LTS" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
            </div>
        </div>
