        ...
        <div class="page-card">
            <form method="POST" action="{{ route('incidents.store') }}">
                ...
                @if (isset($selectedReservation))
                    ...
                    @ <div
                        class="reservation-incid">
                        <strong>Linked to Reservation #{{ $selectedReservation->id }}</strong><br>
                        <small>Resource: {{ $selectedReservation->resource->name }}</small>
                    </div>
                @endif
                ...
                <div class="filter-group" style="margin-bottom: 20px;">
                    ...
                    <input type="text" name="title" class="filter-input" required
                        placeholder="Brief summary of the issue ( Just to use it as a Title)">
                    @error('title')
                ...
                <div class="filter-group" style="margin-bottom: 20px;">
                    <label style="display:block; margin-bottom:8px; font-weight:600; color:#64748b;">Affected
                        Resource</label>
                    <select name="resource_id" class="filter-input" required>
                        <option value="" disabled selected>Select a resource</option>
                        @foreach ($resources as $resource)
                            <option value="{{ $resource->id }}"
                                {{ isset($selectedReservation) && $selectedReservation->resource_id == $resource->id ? 'selected' : '' }}>
                                {{ $resource->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="filter-group" style="margin-bottom: 30px;">
                    <label style="display:block; margin-bottom:8px; font-weight:600; color:#64748b;">Issue
                        Description</label>
                    <textarea name="description" rows="4" class="filter-input" required
                        placeholder="Describe the technical issue you are experiencing..."></textarea>
                    @error('description')
                        <div class="error-msg">{{ $message }}</div>
                    @enderror
                </div>
